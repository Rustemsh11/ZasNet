...
/// <summary>
/// Обработчик команды для отправки отчета по заработку сотрудника в Telegram
/// </summary>
public class SendEmployeeEarningReportToTelegramHandler(
    IRepositoryManager repositoryManager,
    IEmployeeEarningReportService reportService,
    ITelegramBotAnswerService telegramBotAnswerService)
    : IRequestHandler<SendEmployeeEarningReportToTelegramCommand>
{
    public async Task Handle(
        SendEmployeeEarningReportToTelegramCommand request,
        CancellationToken cancellationToken)
    {
        ...
        var employeeId = firstRecord.Employee.Id;

        // Получаем информацию о сотруднике из БД для получения ChatId
        var employee = await repositoryManager.EmployeeRepository
            .FindByCondition(e => e.Id == employeeId, false)
        ...
        if (employee.ChatId == null)
        {
            throw new InvalidOperationException(
                $"У сотрудника {employee.Name} не привязан Telegram аккаунт");
        }
        ...
        var totalEarning = request.Data.Sum(d => d.EmployeeEarningDto.EmployeeEarning);

        // Формируем имя файла
        var fileName = reportService.GenerateFileName(employee.Name, minDate.Month, minDate.Year);

        // Формируем сообщение
        var caption = $"📊 Отчет по заработной плате\n" +
